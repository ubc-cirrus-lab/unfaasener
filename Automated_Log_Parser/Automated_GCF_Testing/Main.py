...
LargeMessages = int(input("Do you want to test large messages: (Enter 1 for yes and 0 for no)"))
invoker = str(input("Do you want to invoke the function in vm or gcf?"))
function = str(input("If you select vm, please enter the function name, otherwise press enter"))

creatingObj = CreateGCFLogs(num, interval,  testCount, sleepTime, LargeMessages, invoker, function)
...
vmData = (input("Enter the path to the subscriber's data file if you chose vm,otherwise press enter"))
