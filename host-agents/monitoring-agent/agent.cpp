...
void callScheduler(const std::string &mode)
{
        // API endpoint configuration
        std::string leader_host_ip = "0.0.0.0";
        std::string apiEndpoint = "http://" + leader_host_ip + ":1111/run_scheduler";
        std::string postData = "{\"mode\": \"" + mode + "\"}";

        // API request
        std::string command = "curl -X POST -H \"Content-Type: application/json\" -d '" + postData + "' " + apiEndpoint;
        std::system(command.c_str());
}

// Function to write the latest predictions to the file
void writePrediction(double cpu_pred, double mem_pred)
{
        std::ofstream myfile;
        myfile.open("../../zookeeper/src/main/resources/prediction.properties");
        myfile << "cpu=" << cpu_pred << "\n";
        myfile << "mem=" << mem_pred << "\n";
        myfile.close();
}
