...
class Estimator:
    ...
    def getLastObservations(self):
        for func in self.workflowFunctions:
            selectedReq = self.dataframe.loc[(self.dataframe["reqID"] == self.finalRecodreqID) & (self.dataframe["function"] ==  func)]
            selectedReqForFunc = selectedReq.iloc[0]["start"]
            startDate = selectedReqForFunc
            # startDate = datetime.date((selectedReqForFunc["start"])
            self.dataframe['start'] = pd.to_datetime(self.dataframe['start'], format="%Y-%m-%d %H:%M:%S.%f")
            # self.dataframe
            selected = self.dataframe.loc[(self.dataframe['function'] ==  func) & (self.dataframe["host"] == "s") & ((self.dataframe["start"]) < startDate)]
            selected = self.dataframe.drop(self.dataframe[(self.dataframe['function'] ==  func) & (self.dataframe["host"] == "s") & ((self.dataframe["start"]) < startDate)].index, inplace = True)
            print("func:::", func)
            print(selected)
