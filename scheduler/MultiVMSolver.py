...
class OffloadingSolver:
    ...
    def __init__(self,workflow, mode, decisionMode, toleranceWindow):
        with open((os.getcwd()+"/data/" + str(workflow)+ "/"+ 'slackData.json'), 'r') as outfile:
            self.slacksDF = json.load(outfile) 
        with open((os.getcwd()+"/data/" + str(workflow)+ "/"+ 'slackDurations.json'), 'r') as outfile:
            self.slackDurationsDF = json.load(outfile) 
        with open((os.getcwd()+"/data/" + str(workflow)+ "/"+ 'pubSubSize.json'), 'r') as outfile:
            self.pubSubSize = json.load(outfile) 
        with open((os.getcwd()+"/data/" + str(workflow)+ "/"+ 'Costs.json'), 'r') as outfile:
    ...
    def addedExecLatency(self, offloadingCandidate, vm):
        ...
        serverless = self.estimator.getFuncExecutionTime(offloadingCandidate, "s", self.decisionMode)
        vm = self.estimator.getFuncExecutionTime(offloadingCandidate, ("vm"+str(vm)), self.decisionMode)
        diff = vm - serverless
        return diff
    ...
    def suggestBestOffloadingMultiVM(self, availResources, alpha, verbose):
        ...
        if self.optimizationMode == "cost":
            ...
            offloadingDecisions = [[model.Var(lb=0,ub=1,integer=True, name = ("function:"+ str(j)+ " resource:" + str(i))) for i in range(len(availResources))] for j in range(len(offloadingCandidates))]
           ...
           # Constraint on only having a single or zero one for each vm decision for a function
            ...
            for path in self.allPathsSlack:
                model.Equation( sum([sum([( (offloadingDecisions[node][vm]*self.allPathsSlack[path][node]*(self.addedExecLatency(offloadingCandidates[node], vm))) + ((sum([((self.allPathsSlack[path])[node])*(model.abs2(offloadingDecisions[node][vm]-offloadingDecisions[j][vm]))*((self.allPathsSlack[path])[j])*self.addedComLatency((offloadingCandidates[j]), (offloadingCandidates[node])) for j in self.getParentIndexes(offloadingCandidates[node])]))) )for node in range(len(offloadingDecisions))])for vm in range(len(availResources))]) <= ((self.getCriticalPathDuration() - path) + self.toleranceWindow))
