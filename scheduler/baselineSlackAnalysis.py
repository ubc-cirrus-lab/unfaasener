    ...
    def getObservations(self):
        for func in self.workflowFunctions:
            for reqID in self.selectedIDs:
                df2=self.dataframe.loc[((self.dataframe["reqID"] == reqID) & (self.dataframe["function"] == func))]
                if df2.shape[0] == 1:
        ...
        for entry in self.slackData.keys():
            if entry not in self.workflowFunctions:
                for reqID in self.selectedIDs:
                    ...
                    dfPrev=self.dataframe.loc[((self.dataframe["reqID"] == reqID) & (self.dataframe["function"] == prevFunc))]
                    dfNext=self.dataframe.loc[((self.dataframe["reqID"] == reqID) & (self.dataframe["function"] == nextFunc))]
                    if (dfPrev.shape[0] == 1):
                    ...
                    if (dfNext.shape[0] == 1):
                        ...
                        start = dfNext.loc[dfNext["mergingPoint"] == prevFunc].iloc[0]["start"]
