...
def trainMain(event, context):
    ...
    if routing == "0":
          ...
          publish_future = publisher.publish(topic_path, data=message_bytes, publishTime = str(datetime.datetime.utcnow()), identifier = msgID, reqID = (event['attributes'])['reqID'], routing = routingData.encode("utf-8"), messageContent = "firstModel", branchName = "RegressionTuning", branch = "RegressionTuning_firstModel")
        ...
        publish_future = publisher.publish(topic_path, data=message_bytes, publishTime = str(datetime.datetime.utcnow()), identifier = msgID, reqID = (event['attributes'])['reqID'],invokedFunction = invokedFunction, routing = routingData.encode("utf-8"), messageContent = "firstModel", branchName = "RegressionTuning", branch = "RegressionTuning_firstModel")
        publish_future.result()
