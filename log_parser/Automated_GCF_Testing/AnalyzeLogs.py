...
class AnalyzeLogs:
        ...
        if self.subType == "gcf":
            for id in self.execodes:
                ...
                for entry in self.logs:
                    if (
                        entry["execution_id"] == id
                        and entry["log"] == "Function execution started"
                    ):
                        index = self.logs.index(entry)
                        for log in self.logs[:index]:
                            if log["execution_id"] == id and (
                                "WARNING:root:" in log["log"]
                            ):
                                identifier = log["log"]
                        ...
                        for log in self.logs[:index]:
                            if (identifier in (log["log"])) and (
                                log["name"] == "secondSubscriber"
                            ):
                                ...
                                for attr in attributes:
                                    if "publishedTime" in attr:
                                        dateFinishedLog = attr.replace(
                                            "publishedTime:", ""
                                        )
                                        if dateFinishedLog.endswith("Z"):
                                            dateFinishedLog = (
                                                dateFinishedLog[:-1] + ".000"
                                            )
                                        dateFinish = datetime.datetime.strptime(
                                            dateFinishedLog, "%Y-%m-%d %H:%M:%S.%f"
                                        )
                                        endFlag = True
                                ...
                                for each in self.logs[(subscriberIndex + 1) :]:
                                    if (
                                        (each["log"]) == "Function execution started"
                                        and each["name"] == "secondSubscriber"
                                        and each["execution_id"] == self.subscriberExeID
                                    ):
    ...
    def createPlot(self, logsDataFrame):
        ...
        ax.plot(
            logsDataFrame.groupby(["Bytes"]).groups.keys(),
            meanDF.Latency,
            color="#f05988",
            marker="o",
            label="Mean Latency",
    ...
    def saveData(self, df):
        ...
        with open(
            os.getcwd() + "/data/" + "Latency-Bytes, " + runningTime + ".json", "w"
        ) as latencyFile:
            json.dump(df, latencyFile)

    def plotEachGp(self, df):
        for gp in df.groupby(["Bytes"]).groups.keys():
            axarr = ((df.groupby(["Bytes"])).get_group(gp)).hist(
                cumulative=True, column=["Latency"], density=1
            )
