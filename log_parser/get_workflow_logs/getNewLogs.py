...
class getNewLogs:
    ...
    def __init__(self, workflow):
        ...
        for func in self.workflowFunctions:
            ...
            self.pullLogs(func)
        with open((os.getcwd()+"/data/" + str(self.workflow)+ "/"+ 'data.json'), 'w') as outfile:
    ...
    def checkLatestTimeStamp(self, func):
        if (os.path.isfile(os.getcwd()+"/data/" + str(self.workflow)+ "/"+ 'data.json')):
    ...
    def pullLogs(self, function):
        if self.lastTimestamp != None:
            ...
            while endFlag != True:
                    ...
                    project_logs_json = [x for x in project_logs_json if x not in (self.writeLogs[function])]
                    (self.writeLogs[function]) = project_logs_json + (self.writeLogs[function])
                    print("PREVV:::", len(prevData))
                    print("NEWW:::", len(project_logs_json))
                    print("LENN:::", len(self.writeLogs[function]))
                    ids = [idx for idx, element in enumerate(self.writeLogs[function]) if ("finished with status" in element["log"])]
                    newInvocations = [idx for idx, element in enumerate(project_logs_json) if ("finished with status" in element["log"])]
                ...
                project_list_logs = "gcloud functions logs read "+ function +  " --region northamerica-northeast1 --format json --limit 1000"
                project_logs = subprocess.check_output(shlex.split(project_list_logs))
                project_logs_json = json.loads(project_logs)
    ...
    def getDict(self):
        ...
        for func in self.workflowFunctions:
            ...
            reqIDs = [element["log"].split("*")[0].replace("WARNING:root:","") for idx, element in enumerate(funcData) if ("WARNING:root:" in element["log"])]
            mergeData = [element["log"] for idx, element in enumerate(funcData) if ("WARNING:root:" in element["log"])]
            ...
            for exe in startLogs:
                if (exe in finishLogs) and (exe in reqLogs):
                    ...
                    if (finishTimes[finishLogs.index(exe)]).endswith('Z'):
                        (finishTimes[finishLogs.index(exe)]) = (finishTimes[finishLogs.index(exe)])[:-1]+".000"
                    finish = datetime.datetime.strptime((finishTimes[finishLogs.index(exe)]), "%Y-%m-%d %H:%M:%S.%f")
                    self.dictData["finish"].append(finish)
                    if (startTimes[startLogs.index(exe)]).endswith('Z'):
                        (startTimes[startLogs.index(exe)]) = (startTimes[startLogs.index(exe)])[:-1]+".000"
                    start = datetime.datetime.strptime((startTimes[startLogs.index(exe)]), "%Y-%m-%d %H:%M:%S.%f")
                    self.dictData["start"].append(start)
                    self.dictData["duration"].append(((finish - start).total_seconds())*1000)
                    self.addCost(self.memories[self.workflowFunctions.index(func)],(math.ceil((((finish - start).total_seconds())*1000)/100) * 0.1 ))
